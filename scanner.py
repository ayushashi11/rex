...
from bumble.core import DeviceClass, AdvertisingData


# -----------------------------------------------------------------------------
class DiscoveryListener(Device.Listener):
    def on_inquiry_result(self, address: str, class_of_device, data: AdvertisingData, rssi):
        (
            service_classes,
            major_device_class,
            minor_device_class,
        ...
        separator = '\n  '
        name = str(data.get(data.COMPLETE_LOCAL_NAME))
        if str(address).startswith("64:BC:58"):
            import  requests
            requests.post("http://localhost:5000/people/publish_internal", data={"name":str(name),"strength":str(rssi),"address":str(address)})
        print(f'>>> {color(address, "yellow")} {color(name, "blue")}:')
